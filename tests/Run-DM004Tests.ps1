...
param([switch]$SkipApply, [switch]$SkipAwsTests, [switch]$RunPublish, [switch]$SkipAthena)
...
Write-Host "`n=== DM-004 Automated Validation Suite ===`n" -F Cyan
Push-Location (Join-Path $Root "terraform"); terraform fmt -check -recursive 2>$null; Step "terraform fmt -check" ($LASTEXITCODE -eq 0); Pop-Location
...
if (-not $SkipAwsTests) {
    $a = @("-File",(Join-Path $PSScriptRoot "Validate-DM004Aws.ps1"))
    if ($RunPublish) { $a += "-RunPublish" }; if ($SkipAthena) { $a += "-SkipAthena" }
    & powershell @a; Step "Validacao AWS DM-004" ($LASTEXITCODE -eq 0)
}
