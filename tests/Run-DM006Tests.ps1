...
param([switch]$SkipApply, [switch]$SkipAwsTests, [switch]$SkipHttp)
...
Write-Host "`n=== DM-006 Automated Validation Suite ===`n" -F Cyan
...
if (-not $SkipAwsTests) {
    $a = @("-File",(Join-Path $PSScriptRoot "Validate-DM006Aws.ps1"))
    if ($SkipHttp) { $a += "-SkipHttp" }
    & powershell @a; Step "Validacao AWS DM-006" ($LASTEXITCODE -eq 0)
}
